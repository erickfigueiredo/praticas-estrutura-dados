...
public:
	...
	//Declaramos MyList2Iterator como friend para que ela consiga acessar o dataLast de seu membro de dados "list"
	template<class T2>
	friend class MyList2Iterator;
	...
	int size() const;
	int size(const Node<T> *first) const;
private:
...
public:
	...
	//Resposta: Não, não faz sentido pois os elementos da lista não estão alocados em ordem na memória.

	//exercicio: armazenamos um ponteiro nesta classe. Deveriamos ter destrutor, constr. de copia, etc?
	//Não pois não há uma alocação dinâmica de memória em nossa classe.
...
template<class T>
MyList2Iterator<T> MyList2Iterator<T>::operator--() {
	//Se o ponteiro do nosso objeto iterator apontar para NULL
	if(ptr == NULL)
		//Se o ponteiro do nosso objeto for NULL, pegamos o último elemento da lista e atribuímos para nosso ponteiro  
		ptr = list->dataLast;
	else
		ptr = ptr->prev;		
...
MyList2Iterator<T> MyList2Iterator<T>::operator++(int) {
	...
	//Dereferenciamos o objeto e fazemos o predecremento
	++(*this);
...
MyList2Iterator<T> MyList2Iterator<T>::operator--(int) {
	...
	//Dereferenciamos o objeto e fazemos o preincremento
	--(*this);
...
template <class T>
int MyList2<T>::size(const Node<T> *first)const{
	if(first == NULL)
		return 0;
