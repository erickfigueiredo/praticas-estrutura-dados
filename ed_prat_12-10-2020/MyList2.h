...
private:
	...
	int size(const Node<T> *last) const;
...
void MyList2<T>::insert(const T&elem, iterator where) {
	if(where == NULL){
		push_back(elem);
	}else if (where->prev == NULL){
		push_front(elem);
	}else{
		iterator it = new Node<T>(elem);

		it->prev = where->prev;
		it->next = where;
		where->prev = where->prev->next = it;
	}
...
template<class T>
int MyList2<T>::size(const Node<T> *last) const{
	if(last == NULL) 
		return 0;
	if(last->prev == NULL) 
		return 1;

	return 1 + size(last->prev);
}
...
int MyList2<T>::eraseMatchingElements(const T& elem){
	//Caso especial, não temos elementos na lista
	if(dataFirst == NULL)
		return 0;
	...
	for(int i = 0; it != NULL; i++){
		if(it->data == elem){
			...
			c++;
			if(dataFirst == it){
				//Segundo caso especial, temos apenas um elemento
				if(dataSize == 1){
					delete dataFirst;
					delete dataLast;

					it = dataFirst = dataLast = NULL;
					c++;

					break;
				}

				iterator itNext = dataFirst->next;
				itNext->prev = NULL;
...
template<class T>
void MyList2<T>::reverse(Node<T> *ptr){
...
template <class T>
bool MyList2<T>::compare(const MyList2<T>::iterator it1, const MyList2<T>::iterator it2) const{
	if(it1 == NULL || it1->next == NULL)
		return false;
