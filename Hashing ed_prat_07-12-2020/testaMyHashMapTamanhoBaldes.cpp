...
class MyStringHash2 {
public:
	unsigned operator()(const string &s) const  { //Organiza pela última letra
		if (s.size()-1)
...
class MyStringHash3 {
public:
	unsigned operator()(const string &s) const { //Organiza pelo caractere do "meio"
		if ((s.size()-1)/2 >= 0)
...
class MyStringHash4 {
public:
	unsigned operator()(const string &s) const { //Organiza pelo segundo caractere, se tiver apenas um, organiza pelo primeiro
		if(s.size() >1)
			return (int)s[1];
