...
void reverse(MyList2<T> &lista) {
  typename MyList2<T>::iterator aux1 = lista.begin();
  typename MyList2<T>::iterator aux = aux1;
  
  int size = 0;
  
  //Utilizamos dois iteradores, aux1 deve chegar até NULL para que aux receba o último elemento da lista
  //Além disso ao final da iteração vamos obter o size da lista
  for(size; aux1 != lista.end(); size++){
    aux = aux1;
    aux1++;
  }

  typename MyList2<T>::iterator inicio = lista.begin();
  typename MyList2<T>::iterator fim = aux;

  T auxData;

  //Realizamos size/2 iterações sobre a lista incrementando inicio e decrementando fim, enquanto trocamos suas posições
